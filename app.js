...
async function syncScoresFromESPN(showFeedback) {
  ...
  // Fetch ESPN for a ±7-day window around today (ART).
  // This covers recent results AND upcoming scheduled matches (ESPN publishes
  // teams for confirmed fixtures even before they're played).
  const artNow = new Date(Date.now() - 3 * 60 * 60 * 1000);
  const windowStart = new Date(artNow);
  windowStart.setUTCDate(windowStart.getUTCDate() - 7);
  const windowEnd = new Date(artNow);
  windowEnd.setUTCDate(windowEnd.getUTCDate() + 7);
  const windowStartStr = windowStart.toISOString().slice(0, 10);
  const windowEndStr   = windowEnd.toISOString().slice(0, 10);
  const dates = [...new Set(
    MATCHES
      .filter(m => m.date >= windowStartStr && m.date <= windowEndStr)
    ...
    for (const dateStr of dates) {
      ...
      for (const event of (data.events || [])) {
        ...
        if (espnHome && espnAway && isFinished) {
          ...
          // No es grupo — puede ser eliminatoria, continúa al bloque knockout
        }
