...
function renderAdminView() {
  ...
  _syncInterval = setInterval(() => syncScoresFromESPN(false), 5 * 60 * 1000);
}
  ...
  try {
    for (const dateStr of dates) {
      ...
      for (const event of (data.events || [])) {
        ...
        // If ESPN has teams reversed vs our MATCHES entry, swap scores
        if (match.home === espnAway && match.away === espnHome) {
          [hScore, aScore] = [aScore, hScore];
        }

        // Skip if already stored with same values
        const existing = State.remoteMatches[match.id];
        if (existing?.status === 'finished' &&
            existing.homeScore === hScore &&
            existing.awayScore === aScore) continue;
